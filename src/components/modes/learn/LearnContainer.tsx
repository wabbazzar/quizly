...
const LearnContainer: FC<LearnContainerProps> = memo(({
  ...
  // Use existing hooks
  const { generateRound, questions, currentQuestion: generatedQuestion, nextQuestion: nextGeneratedQuestion } = useQuestionGenerator(deck, settings);
  ...
  const handleAnswerSubmit = useCallback((answer: string, isCorrect: boolean, cardIdx: number) => {
  ...
  const handleQuestionComplete = useCallback(() => {
    ...
    if (currentIndex >= questions.length - 1) {
      ...
      const accuracyPercent = totalAnswered > 0
        ? (sessionState.correctCards.size / totalAnswered) * 100
        : 0;
